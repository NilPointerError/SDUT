...
/*
	...
	平衡二叉树的查找、插入和删除的平均和最坏情况下的时间复杂度都是O(logn)。空间复杂度为O(n)
	而二叉排序树的时间复杂度介于O(logn)和O(n)之间，如果退化成单链表，时间复杂度就是顺序查询，为O(n)
	平衡二叉树相比于二叉排序树的优势在于它能够保证树的高度平衡，从而避免二叉排序树在最坏的情况下退化成
	链表，导致查找效率降低。
